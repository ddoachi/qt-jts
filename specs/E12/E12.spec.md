...
│                                                                          │
│  ┌─────────────┐    ┌─────────────┐    ┌─────────────┐    ┌──────────┐ │
│  │  Market     │───►│  Signal     │───►│  Virtual    │───►│  Position│ │
│  │  Data Feed  │    │  Generator  │    │  Executor   │    │  Tracker │ │
│  │             │    │             │    │             │    │          │ │
│  │  Real-time  │    │  Entry/Exit │    │  Simulate   │    │  Track   │ │
│  │  from gRPC  │    │  detection  │    │  fills      │    │  P&L     │ │
│  └─────────────┘    └─────────────┘    └─────────────┘    └──────────┘ │
│        │                   │                   │                  │     │
│        │                   ▼                   │                  │     │
│        │           ┌─────────────┐             │                  │     │
...
├── paper_trading_view.py       # Main paper trading view
├── session_config_widget.py    # Session configuration
├── portfolio_widget.py         # Portfolio display
├── signals_widget.py           # Signal list and actions
├── session_stats_widget.py     # Session statistics
...
class SessionConfig:
    ...
    symbols: list[str]
    virtual_capital: Decimal = Decimal("100000000")  # 1억원
...
│ Status: Running                    Started: 2024-12-20       [일시정지] │
│ Data: Creon (15 req/s)                                                   │
├─────────────────────────────────────────────────────────────────────────┤
│                                                                          │
...
│ Today's Signals:                                                         │
│ ┌────────────────────────────────────────────────────────────────────┐  │
│ │ 09:15 🟢 BUY  035720 카카오    Entry signal triggered    [실행]   │  │
│ │ 10:30 🔴 SELL 005380 현대차    Exit signal triggered     [실행]   │  │
│ └────────────────────────────────────────────────────────────────────┘  │
│                                                                          │
│ Performance:                                                             │
│ ├─ Total Trades: 12                                                      │
...
class PortfolioWidget(QWidget):
    ...
    def _setup_ui(self):
        ...
        header = QHBoxLayout()
        self._value_label = QLabel("Portfolio Value: ₩0")
    ...
    def set_portfolio(self, portfolio: VirtualPortfolio):
        ...
        self._value_label.setText(f"Portfolio Value: ₩{total_value:,.0f}")
        ...
        for i, (symbol, pos) in enumerate(portfolio.positions.items()):
            ...
            self._table.setItem(i, 3, QTableWidgetItem(f"₩{pos.avg_price:,.0f}"))
            self._table.setItem(i, 4, QTableWidgetItem(f"₩{pos.current_price:,.0f}"))

            pnl_item = QTableWidgetItem(
                f"{'+' if pos.unrealized_pnl >= 0 else ''}₩{pos.unrealized_pnl:,.0f} ({pos.unrealized_pnl_pct:+.1f}%)"
            )
...
class SignalsWidget(QWidget):
    ...
    def _create_signal_widget(self, signal: TradingSignal) -> QWidget:
        ...
        if signal.direction == TradeDirection.BUY:
            icon_label = QLabel("🟢 BUY")
        else:
            icon_label = QLabel("🔴 SELL")
